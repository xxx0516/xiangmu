...
object ZoneDimForRDD {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dimRDD: Dataset[((String, String), List[Double])] = df.map(row => {
      ...
      val cyjjs: List[Double] = DIMZhibiao.jingjiaRtp(iseffective, isbilling, isbid, iswin, adorderid, adplatformproviderid)
      ...
      val ggxf: List[Double] = DIMZhibiao.ggcbRtp(iseffective, isbilling, iswin, winprice, adpayment, adplatformproviderid)
